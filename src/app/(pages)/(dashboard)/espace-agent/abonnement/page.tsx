...
function SubscriptionModal({
  ...
  const handleUploadScreenshot = async (file: File) => {
    ...
    setError("");
    try {
      const { key, url } = await presignSubscriptionScreenshot(
        token,
        file.name,
        file.type,
      );
  ...
  const planPrice =
    selectedTier === "PRO" ? t.subPlanProPrice : t.subPlanAgencyPrice;
            ...
            return (
              <div key={idx} className="flex items-center flex-1">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span
                    className={`text-[10px] font-medium ${active ? "text-blue-600" : "text-muted-foreground"}`}
                  >
                    {label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 mb-4 rounded ${done ? "bg-blue-400" : "bg-border"}`}
                  />
                )}
              </div>
            );
              ...
              <div className="flex flex-col gap-3">
                {PLANS.map((plan) => {
                  ...
                  const features = isPro
                    ? [
                        t.subPlanProFeature1,
                        t.subPlanProFeature2,
                        t.subPlanProFeature3,
                      ]
                    : [
                        t.subPlanAgencyFeature1,
                        t.subPlanAgencyFeature2,
                        t.subPlanAgencyFeature3,
                      ];
                  const price = isPro
                    ? t.subPlanProPrice
                    : t.subPlanAgencyPrice;
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${plan.badgeBg} ${plan.badgeText}`}
                          >
            ...
            <>
              <div className="rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 p-4 flex flex-col gap-3">
                ...
                <div className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
                  ...
                  <p>
                    {t.subPayName} : <strong>Okapi Real Estate</strong>
                  </p>
                </div>
                <div className="flex items-center gap-2 bg-white dark:bg-zinc-800 rounded-lg border px-3 py-2.5">
                  ...
                  <span className="font-mono font-bold text-blue-700 text-sm flex-1 tracking-wider">
                    {reference}
                  </span>
                  <button
              ...
              <div>
                ...
                {screenshotUrl ? (
                  ...
                  >
                    ...
                    {screenshotUploading
                      ? t.subScreenshotUploading
                      : t.subScreenshotChoose}
                  </button>
                )}
              </div>
...
function SubscriptionCard({
  ...
  const screenshotUrl = sub.screenshotUrl
    ? resolveUrl(sub.screenshotUrl)
    : null;
      ...
      <div className="flex items-start gap-4 p-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            <div
              className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold ring-1 ${st.badge}`}
            >
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <span>
                {PAYMENT_METHODS.find((m) => m.id === sub.paymentMethod)
...
function EspaceAgentAbonnementPageInner() {
  ...
  // Payments disabled — redirect to dashboard
  useEffect(() => {
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    ...
    {
      key: "ACTIVE",
      label: t.subTabActive,
      count: subs.filter((s) => s.status === "CONFIRMED").length,
    },
    {
      key: "HISTORY",
      label: t.subTabHistory,
      count: subs.filter((s) => ["REJECTED", "EXPIRED"].includes(s.status))
        .length,
    },
  ];
