...
function PropertyThumb({
  ...
  const cfg = CATEGORY_ICON[category?.toLowerCase() ?? ""] ?? {
    Icon: Home,
    bg: "bg-muted",
...
function statusConfig(t: T) {
  return {
    PENDING: {
      label: t.boostStatusPending,
      dot: "bg-yellow-400",
      badge: "bg-yellow-50 text-yellow-700 ring-yellow-200",
    },
    CONFIRMED: {
      label: t.boostStatusConfirmed,
      dot: "bg-green-500",
      badge: "bg-green-50 text-green-700 ring-green-200",
    },
    REJECTED: {
      label: t.boostStatusRejected,
      dot: "bg-red-400",
      badge: "bg-red-50 text-red-700 ring-red-200",
    },
    EXPIRED: {
      label: t.boostStatusExpired,
      dot: "bg-gray-300",
      badge: "bg-gray-50 text-gray-500 ring-gray-200",
    },
  } satisfies Record<
    BoostRequest["status"],
...
function BoostModal({
  ...
  const [selectedPlan, setSelectedPlan] = useState<
    (typeof BOOST_PLANS)[number] | null
  >(null);
  const [selectedMethod, setSelectedMethod] =
    useState<BoostPaymentMethod | null>(null);
  ...
  const handleUploadScreenshot = async (file: File) => {
    ...
    setError("");
    try {
      const { key, url } = await presignBoostScreenshot(
        token,
        file.name,
        file.type,
      );
            ...
            return (
              <div key={idx} className="flex items-center flex-1">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span
                    className={`text-[10px] font-medium ${active ? "text-amber-600" : "text-muted-foreground"}`}
                  >
                    {label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 mb-4 rounded ${done ? "bg-amber-400" : "bg-border"}`}
                  />
                )}
              </div>
            );
            ...
            <>
              <div className="rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 p-4 flex flex-col gap-3">
                <p className="text-sm font-semibold text-amber-800 dark:text-amber-400">
                  {selectedPlan?.amount} $ — {selectedPlan?.durationDays}{" "}
                  {t.boostDays} ·{" "}
                ...
                <div className="text-xs text-amber-700 dark:text-amber-300 space-y-1">
                  <p>
                    {t.boostPayNumber} :{" "}
                    <strong>
                      {process.env.NEXT_PUBLIC_PAYMENT_NUMBER ??
                        "+243 XXX XXX XXX"}
                    </strong>
                  </p>
                  <p>
                    {t.boostPayName} : <strong>Okapi Real Estate</strong>
                  </p>
                </div>
                <div className="flex items-center gap-2 bg-white dark:bg-zinc-800 rounded-lg border px-3 py-2.5">
                  ...
                  <span className="font-mono font-bold text-amber-700 text-sm flex-1 tracking-wider">
                    {reference}
                  </span>
                  <button
              ...
              <div>
                ...
                {screenshotUrl ? (
                  ...
                  >
                    ...
                    {screenshotUploading
                      ? t.boostScreenshotUploading
                      : t.boostScreenshotChoose}
                  </button>
                )}
              </div>
            ...
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <div className="bg-muted rounded-xl px-4 py-3 text-left w-full text-sm space-y-1">
                <p>
                  ...
                  {selectedPlan?.durationDays} {t.boostDays} — $
                  {selectedPlan?.amount}
                </p>
                <p>
                  <span className="text-muted-foreground">
                    {t.boostSummaryPayment}
                  </span>{" "}
                  {PAYMENT_METHODS.find((m) => m.id === selectedMethod)?.label}
                </p>
                <p>
...
function BoostCard({
  ...
  const cover = b.property.gallery?.[0]
    ? resolveUrl(b.property.gallery[0])
    : null;
  const location = [b.property.suburb, b.property.city]
    .filter(Boolean)
    .join(", ");
    ...
    <div className="group bg-white dark:bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      <div className="flex flex-col sm:flex-row gap-0">
        ...
        <div className="sm:w-40 h-36 sm:h-auto relative bg-muted shrink-0">
          ...
          {/* Status overlay */}
          <div
            className={`absolute top-2 left-2 flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold ring-1 ${st.badge}`}
          >
        ...
        <div className="flex-1 p-4 flex flex-col justify-between gap-3">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            ...
            <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <span>
                {b.paymentMethod === "ORANGE_MONEY"
                  ? "Orange Money"
                  : b.paymentMethod === "MTN_MONEY"
                    ? "MTN Money"
                    : b.paymentMethod === "AIRTEL_MONEY"
                      ? "Airtel Money"
            ...
            <div className="flex items-center gap-1.5 text-muted-foreground col-span-2">
              ...
              <span className="font-mono font-semibold text-amber-700 tracking-wider">
                {b.paymentReference ?? "—"}
              </span>
            </div>
              ...
              <div className="flex items-center gap-1.5 text-muted-foreground col-span-2">
                ...
                <span>
                  ...
                  <span className="font-medium text-foreground">
                    {new Date(b.property.boostedUntil).toLocaleDateString(
                      "fr-FR",
...
function EspaceAgentBoostsPageInner() {
  ...
  // Boost payments disabled — redirect to dashboard
  useEffect(() => {
    if (!FEATURE_FLAGS.BOOST_ENABLED) {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated || !token) {
      router.replace("/connexion?agent=1");
      return;
    }
    ...
    if (pid && ptitle)
      setModal({ propertyId: pid, propertyTitle: decodeURIComponent(ptitle) });
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    {
      key: "PENDING",
      label: t.boostTabPending,
      count: boosts.filter((b) => b.status === "PENDING").length,
    },
    {
      key: "ACTIVE",
      label: t.boostTabActive,
      count: boosts.filter((b) => b.status === "CONFIRMED").length,
    },
    {
      key: "HISTORY",
      label: t.boostTabHistory,
      count: boosts.filter((b) => ["REJECTED", "EXPIRED"].includes(b.status))
        .length,
    },
  ];
        ...
        <div className="rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border border-amber-200/60 p-6 flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1">
            <p className="font-bold text-amber-900 dark:text-amber-300">
              {t.boostVisibilityTitle}
            </p>
            <p className="text-sm text-amber-700 dark:text-amber-400 mt-0.5">
              {t.boostVisibilityDesc}
            </p>
          </div>
          <div className="flex gap-2 shrink-0">
            {BOOST_PLANS.map((plan) => (
              <span
                key={plan.id}
                className="text-xs font-semibold bg-white/80 dark:bg-white/10 text-amber-700 dark:text-amber-300 px-2.5 py-1 rounded-full border border-amber-200"
              >
                {plan.durationDays}
                {t.boostDays[0]}/${plan.amount}
              </span>
            ))}
          </div>
        </div>
