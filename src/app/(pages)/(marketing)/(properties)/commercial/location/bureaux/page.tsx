...
export default async function Page({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; type?: string; minPrice?: string; maxPrice?: string; beds?: string }> }) {
  ...
  const raw = await getPropertiesByCategory("commercial", "office", "rent");
  ...
  return (
    <PropertyListingPage
      title="Bureaux à louer à Kinshasa"
      totalListings={all.length}
      mode="commercial"
      crumbs={[{ label: "Commercial", href: "/commercial" }, { label: "Location", href: "/commercial" }, { label: "Bureaux" }]}
      categories={[
        { label: "Bureaux", count: all.length, href: "/commercial/location/bureaux" },
